...
class AniDrawSLTM(object):
    def __init__(self, n_input, n_output, n_steps, n_hidden, batch_size, learning_rate):
        ...
        self.y = tf.placeholder("float", [None, n_steps, n_output])
        # Define weights
        self.weights = {
            # shape (18, 128)
    ...
    def add_input_layer(self,):
        ...
        x_in = tf.reshape(self.x, [-1, self.n_input])
        # shape => (batch_size * timesteps, n_hidden)
        x_in = tf.matmul(x_in, weights['in']) + biases['in']
        # shape => (batch_size, timesteps, n_hidden)
        self.x_in = tf.reshape(x_in, [-1, n_steps, n_hidden])
